...
export function IngredientsDashboard({
  ...
  const openDeleteModal = React.useCallback((ingredient: IngredientResponse) => {
    setSelectedId(ingredient.id);
...
function IngredientForm({
  ...
  const fallbackOption =
    purchaseUnitOptions.find((option) => option.value === initialUnit) ?? purchaseUnitOptions[0];
  ...
  React.useEffect(() => {
    ...
    const option =
      purchaseUnitOptions.find((item) => item.value === nextUnit) ?? purchaseUnitOptions[0];
    setConversion(option ? option.conversionToStock.toString() : '1');
  }, [defaultValues]);

  const handlePurchaseUnitChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
      ...
      <CardContent>
        ...
        >
          ...
          <input
            type="hidden"
            name="stockUnit"
            value={DEFAULT_STOCK_UNIT}
          />
          <input type="hidden" name="supplierId" value={defaultValues?.supplierId ?? ''} />
